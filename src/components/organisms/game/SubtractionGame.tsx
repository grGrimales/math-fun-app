...
export default function SubtractionGame({ difficulty, onExit }: SubtractionGameProps) {
    ...
    const generateProblems = useCallback(() => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            while (options.length < 4) {
                const wrong = Math.floor(Math.random() * config.range) + 1;
    ...
    return (
        <div className="w-full min-h-[70vh] flex items-center justify-center relative overflow-hidden rounded-[3rem]">
            ...
            <Card className="w-full max-w-md mx-auto text-center shadow-2xl border-2 relative z-10 bg-white/95 backdrop-blur-sm">
                <CardHeader className="bg-blue-100 relative"> {/* Color azul para restar */}
                    <button onClick={onExit} className="absolute left-4 top-4 text-blue-500 hover:scale-110 transition-transform">
                        <ArrowLeft size={24} />
                    </button>
                        ...
                        <>
                            <div className="flex justify-between mb-2 text-sm text-muted-foreground font-bold">
                                ...
                                <span className={cn(timeLeft <= 5 ? "text-red-500 animate-pulse" : "")}>
                                    <Timer className="inline-block mr-1" /> {timeLeft}s
                                </span>
                            </div>
                            <Progress value={(current / problems.length) * 100} className="h-2 mb-6" />
                            <div className="text-4xl font-black mb-8 text-text">
